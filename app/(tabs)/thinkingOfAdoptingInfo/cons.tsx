...
function FlipCard({ con, index }: { con: any; index: number }) {
  ...
  return (
    <TouchableOpacity onPress={flip} activeOpacity={1} style={styles.flipContainer}>
      <Animated.View style={[styles.card, { backgroundColor: ORANGE_LIGHT }, { transform: [{ rotateY: frontInterpolate }], opacity: frontOpacity }]}>
        ...
        <View style={styles.tapHint}><Text style={styles.tapHintText}>Tap to learn more</Text></View>
      </Animated.View>
      ...
      <Animated.View style={[styles.card, styles.cardBack, { backgroundColor: WHITE }, { transform: [{ rotateY: backInterpolate }], opacity: backOpacity }]}>
        ...
        <View style={styles.bulletsArea}>
          {[con.support1, con.support2, con.support3, con.support4].filter(s => s && s !== '').map((support, i) => (
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, g) => {
        ...
        if (dx < 0) {
          cons[idx] && cardAnimations[idx].pan.setValue({ x: dx, y: g.dy * 0.1 });
          if (idx < cons.length - 1) {
            const progress = Math.min(Math.abs(dx) / (screenWidth * 0.5), 1);
            cardAnimations[idx + 1].scale.setValue(0.95 + progress * 0.05);
          }
        } else if (dx > 0) {
          cons[idx] && cardAnimations[idx].pan.setValue({ x: dx * 0.18, y: g.dy * 0.05 });
        }
      },
      ...
      onPanResponderRelease: (_, g) => {
        ...
        if (g.dx < 0 && swipedLeft) {
          ...
          Animated.parallel([
            ...
            idx < cons.length - 1 ? Animated.spring(cardAnimations[idx + 1].scale, { toValue: 0.95, useNativeDriver: true, friction: 5 }) : null
...
const styles = StyleSheet.create({
  ...
  reviewBtn: { backgroundColor: ORANGE, paddingVertical: 14, paddingHorizontal: 36, borderRadius: 50, borderBottomWidth: 4, borderBottomColor: ORANGE_DARK },
